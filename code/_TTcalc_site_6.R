  ...
  if(tt_one$volt %>% length > 2){
    ...
    for (ci in tt_one$charge %>% unique) {
      ...
      if (real_time %>% length > 1){
        ...
        tt_one$time = as.numeric(tt_one$time)
        #tt_one$time = as.POSIXct(tt_one$time , origin="1970-01-01 00:00:00",
        #                         tz="Europe/Moscow")
        print(class(tt_one$time))
        print(class(temp_data$time))
        #tt_one$time  = as.Date(tt_one$time)
