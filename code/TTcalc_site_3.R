...
TTBasicCalc = function(tdt){
  ...
  #Dits voltage recalibration
  tdt$volt      = (1.1*131072 / tdt$X8)+0.65
  sharp = c(36460,35994,35121,35817,34810,34718,32394,32462,32809,32822,32712,31277,31224,31328,31326,31376,31337,31196,
            30957, 30880,30714,31000, 31100,30900,31100,30900)
  volt = c(3.35, 3.35,3.38,3.38,3.41,3.41,3.51,3.51,3.51,3.51,3.51,3.8,3.8,3.8,3.8,3.8,4.1,4.1,4.1,4.1,4.1, 4.3, 4.3, 4.3, 4.6, 4.6)
  fit = data.frame(sharp,volt)
  poly_model <- lm(sharp ~ poly(volt,degree=5),data = fit)
  x <-  with(fit,seq(min(volt), max(volt), length.out=2000))
  ndf = data.frame(sharp = x, volt = x)
  y <- predict(poly_model,newdata=ndf)
  y2 = 31000/y
  poly_model2 <- lm(y2 ~ poly(x,degree=3),data = fit)
  dist10c = predict(poly_model2, newdata = data.frame(y2=tdt$volt,x=tdt$volt))
  ...
  tdt$dist4      =  0.000000000000000005964546138691*(tdt$X7^4) -
    0.000000000001582508105016220000*(tdt$X7^3) +
    0.000000163685493685392000000000*(tdt$X7^2) -
    0.008274770831101820000000000000*(tdt$X7) +
    194.856731014274000000000000000000
  tdt$dist4[tdt$volt<3.8] = (-0.5108*tdt$volt^2 + 3.9572*tdt$volt - 6.6611)*tdt$dist4
  tdt$dist5      =  0.000000000000000005964546138691*(tdt$bat_corr^4) -
    0.000000000001582508105016220000*(tdt$bat_corr^3) +
    0.000000163685493685392000000000*(tdt$bat_corr^2) -
    0.008274770831101820000000000000*(tdt$bat_corr) +
    194.856731014274000000000000000000
  tdt$dist6      =  0.000000000000000005964546138691*(tdt$bat_corr^4) -
    0.000000000001582508105016220000*(tdt$bat_corr^3) +
    0.000000163685493685392000000000*(tdt$bat_corr^2) -
    0.008274770831101820000000000000*(tdt$bat_corr) +
    194.856731014274000000000000000000
  tdt$dist7 = 18.2823-0.0006*tdt$X7+0.0000000069143*tdt$X7^2-0.000000000000030237*tdt$X7^3
  tdt$dist8 = 18.2823-0.0006*tdt$X7/dist10c+0.0000000069143*(tdt$X7/dist10c)^2-0.000000000000030237*(tdt$X7/dist10c)^3
  tdt$dist9 = 18.2823-0.0006*tdt$X7*(32830-430.18*tdt$volt)/31000 +0.0000000069143*(tdt$X7*(32830-430.18*tdt$volt)/31000)^2-0.000000000000030237*(tdt$X7*(32830-430.18*tdt$volt)/31000)^3
  tdt$voltshift = tdt$X7 + 1009594.61*tdt$volt^4 - 15124261.00*tdt$volt^3 + 84844737.59*tdt$volt^2 - 211249757.05*tdt$volt + 196980003.60 -3200
  tdt$voltshift2 = tdt$X7 + 1009594.61*tdt$volt^4 - 15124261.00*tdt$volt^3 + 84844737.59*tdt$volt^2 - 211249757.05*tdt$volt + 196980003.60
  tdt$dist10 = 18.2823-0.0006*tdt$voltshift+0.0000000069143*tdt$voltshift^2-0.000000000000030237*tdt$voltshift^3
  tdt$dist11 = 18.2823-0.0006*tdt$voltshift2+0.0000000069143*tdt$voltshift2^2-0.000000000000030237*tdt$voltshift2^3
  tdt$dist12      =  0.000000000000000005964546138691*(tdt$voltshift^4) -
    0.000000000001582508105016220000*(tdt$voltshift^3) +
    0.000000163685493685392000000000*(tdt$voltshift^2) -
    0.008274770831101820000000000000*(tdt$voltshift) +
    194.856731014274000000000000000000
  tdt$dist13      =  0.000000000000000005964546138691*(tdt$voltshift2^4) -
    0.000000000001582508105016220000*(tdt$voltshift2^3) +
    0.000000163685493685392000000000*(tdt$voltshift2^2) -
    0.008274770831101820000000000000*(tdt$voltshift2) +
    194.856731014274000000000000000000
  ...
  if (any(names(tdt)=="s")){
    names(tdt_a)[c(23:25,27)] = c("datetime","rec_num","serv_datetime","id")
  } else {
    names(tdt_a)[c(22:24,26)] = c("datetime","rec_num","serv_datetime","id")
  }
...
check_measure_period = function(temp_data){
  ...
  if(real_time%>% length >1){
    ...
    cat("Estimated step time is ",step_time)
    if(step_time < 4000) {step_time=3600} 
    if(step_time > 3999) {step_time=5400}
    cat(" will be used step time ", step_time, "\n")
...
extrapolate_tt_date = function(tt_one){
  cat("Starting data extrapolation \n")
  if(tt_one %>% nrow <2 ){
    print(tt_one)
  }
  if(tt_one$volt %>% length > 2){
    cat("Starting time correction for ", tt_one$id %>% unique,"\n")
    bat_grow = c(F,
                 tt_one$volt[2:(nrow(tt_one))] - tt_one$volt[1:(nrow(tt_one)-1)] > 0.3)
    cat("Found ",which(bat_grow) %>% length,"recharges","\n")

    tt_one$charge = cumsum(bat_grow)+1
    cat("So, chrges states are", tt_one$charge %>% unique(),"\n")
    ...
    for (ci in tt_one$charge %>% unique){
      ...
      cat("Correct dates ",which(!temp_data$wrong_time) %>% length,
      ...
      step_time = check_measure_period(temp_data)
      tt_one$step_time = step_time
      cat("Charge:",ci," step time:",step_time,
      ...
      if (real_time %>% length > 1){
        ...
        cat("Found measurements with correct timestemp:",
            length(real_time)-1, "\n")
        ...
        cat(tt_one$id %>% unique(), " charge ", ci,"was filled","\n")
...
correct_extrap_date = function(tt_one){
  cat("TT ", tt_one$id %>% unique, " fixing wrong corrections","\n")
   ...
   cat("Found ",which(bad_extrap_index)%>% length," bad extrapolation, fixing","\n" )
   ...
   cat("Removed incorrect time indexes","\n")
   #Restart extrapolation
   for (ci in tt_one$charge %>% unique){
     temp_data = tt_one %>% filter(charge == ci)
     if(!any(is.na(temp_data$time))) {
       cat("Nothing to correct in ",ci," charge, skipping","\n")
       next(ci)
     ...
     if (real_time %>% length > 1){
       ...
       cat("Found measurements with correct timestamp:",
           length(real_time)-1, "\n")
       for( i in 1:(length(real_time)-1)) {
         if(real_time[i+1] == real_time[i]+1){
           next(i)
...
correct_time_ts_shift_matching = function(data){
    #data = tt_data_ec
    ...
    dataec = datac  %>% group_by(id) %>%
      do(correct_extrap_date(.)) %>% as.data.frame
    return(dataec)
...
TTcalc_site = function(server,
  ...
  for (serv_url in server){
    ...
    dest_file_name = paste("temp_",i ,".file", sep="")
    download.file(url = serv_url, destfile = dest_file_name)
    temp_read = suppressWarnings(suppressMessages(
      read_delim(dest_file_name, col_names = FALSE, comment = "#",
                 delim=";", col_types = "ccdddddddddddddddddd")
    ))
    temp_read$s = i
    temp_serv_dataset = rbind( temp_serv_dataset,temp_read)
    cat(serv_url, "data read.","\n")
  }

  tt_data = TTBasicCalc(temp_serv_dataset)
  tt_basic_data = tt_data
  #If there are duplicates - remove
  tt_data = tt_data[-(tt_data[,c(8:32)] %>% duplicated %>% which),]
  ...
  cat("Checking if there are some back up data data? \n")
  if(!is.null(import_folder_name)){
    cat("Yep, there are some \n")
      ...
      for (ids in unique(tt_data$id)){
        ...
        cat("Data from the TT ", ids," imported has ",
            dim(bc)[1]," - rows, ",dim(bc)[2]," - columns.","\n" )
        cat("Data from the TT ", ids," on server has ",
            dim(tt)[1]," - rows, ",dim(tt)[2]," - columns.","\n" )
        cat("Starting import of directly extracted from TT data","\n")
        u = tt
        ...
        if(length(tt$volt)>0){
          ...
          badnames = c("datetime","wrong_time","id","type","incr","rec_num",
                       "serv_datetime","dist4","source","id_rec",
          ...
          if(dim(bc)[1]>10){ #if imported data from tt is to small in most cases it's crappy and better just to skip it
              ...
              if(any(edges == -1) & any(edges == 1) ) {
                ...
                cat("Case 1 intersect, start is ", starts," end is ",ends,"bc size ",dim(bc)[1]," tt size ",dim(tt)[1],"\n")
              } else {
                if(any(edges == -1)){
                  starts = which(edges==-1)
                  ...
                  cat("Case 2 add to end, start is ", starts,"bc size ",dim(bc)[1]," tt size ",dim(tt)[1],"\n")
                }
                if(any(edges == 1)){
                  ends = which(edges==1)
                  ...
                  cat("Case 3 add to begin, end is ",ends,"bc size ",dim(bc)[1]," tt size ",dim(tt)[1],"\n")
              ...
              cat("Case 0 no backup","\n")
            }
            u=frank
            #matches %>% print
            #cat("Found ", matches %>% length,
          ...
          cat("Case 4, only backup result is ", dim(u)[1])
        }
  ...
  tt_data_e = tt_imported  %>% group_by(id) %>%
    do(extrapolate_tt_date(.)) %>% as.data.frame
  tt_data_ec = tt_data_e  %>% group_by(id) %>%
    do(correct_extrap_date(.)) %>% as.data.frame
  tt_data_ec = correct_time_ts_shift_matching(tt_data_ec)
  ...
  SITEdata$b_R_650c = SITEdata$b_R_650*0.7829+202.77
  SITEdata$b_O_600c = SITEdata$b_O_600*0.8654-328.08
  SITEdata$b_Y_570c = SITEdata$b_Y_570*1.0462-666.72
  SITEdata$b_G_550c = SITEdata$b_G_550*1.0546-842.1
  SITEdata$b_B_500c = SITEdata$b_B_500*0.6257-232.13
  SITEdata$b_V_450c = SITEdata$b_V_450*0.4562-212.62
  
  SITEdata$b_W_860c = SITEdata$b_W_860*0.5319+334.88
  SITEdata$b_V_810c = SITEdata$b_V_810*0.8414+91.58
  SITEdata$b_U_760c = SITEdata$b_U_760*1.4549-1012.5
  SITEdata$b_T_730c = SITEdata$b_T_730*1.6209-1511.2
  SITEdata$b_S_680c = SITEdata$b_S_680*1.5199-561.56
  SITEdata$b_R_610c = SITEdata$b_R_610*1.6699-312.45


  return(
    list(tt_imported, SITEdata)
  )

}
#Exporting site data to excel===================================================
export_all_to_excel = function(AllData) {
  var_list = c("SiteIndex", "Species","age_group_index","load","insite_load","VTAscore","id","time","diam","tair","rh","VPD",
               "theta","psi","phi","g2","nt1","NDVI","W","Flux")
  
  AllData = AllData %>% mutate(g2 = gz2+gy2+gx2) 
  AllData = AllData %>% mutate(W = mean((46000-Hz)/(Hz+46000)*50, na.rm=T))
  
  for(site in AllData$SiteIndex %>% unique()){
...
export_site_to_excel = function(site_object,sitename="site",
  ...
  var_list = c("time","id","Species","diam","rec_num","tair","rh","VPD",
               "theta","psi","phi","gz2","nt1","NDVI","EVI","VARI","PRI","Rr",
               "Br","Gr","Flux")
  list_of_datasets = list()
